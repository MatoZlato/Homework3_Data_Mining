import streamlit as st
import pandas as pd

st.set_page_config(page_title="Brand Reputation Monitor", layout="wide")

# Nalaganje podatkov
@st.cache_data
def load_data():
    df = pd.read_csv('podatki_2023.csv')
    df['Datum'] = pd.to_datetime(df['Datum'])
    return df

df = load_data()

# Navigacija
st.sidebar.title("Navigacija")
choice = st.sidebar.radio("Izberite razdelek:", ["Products", "Testimonials", "Reviews"])

if choice in ["Products", "Testimonials"]:
    st.title(f"Prikaz razdelka: {choice}")
    st.dataframe(df, use_container_width=True)

elif choice == "Reviews":
    st.title("🔎 Analiza mnenj (Reviews)")
    
    month_num = st.select_slider(
        "Izberite mesec v letu 2023:",
        options=list(range(1, 13)),
        value=6,
        format_func=lambda x: pd.to_datetime(f"2023-{x}-01").strftime('%B')
    )

    df_filtered = df[df['Datum'].dt.month == month_num].copy()

    if not df_filtered.empty:
        # Gumb za zagon analize (da RAM ne poskoči takoj ob kliku na slider)
        if st.button("Zaženi AI analizo mnenj"):
            with st.spinner('AI analizira... prosim počakajte (lahko traja do 1 minute)'):
                # UVOZI SAMO OB KLIKU - To prihrani ogromno RAM-a ob zagonu!
                from transformers import pipeline
                import plotly.express as px
                
                # Uporabimo ultra majhen model
                sentiment_pipeline = pipeline("sentiment-analysis", model="pysentimiento/robertuito-sentiment-analysis")
                
                results = sentiment_pipeline(df_filtered['Komentar'].tolist())
                df_filtered['Sentiment'] = [res['label'] for res in results]
                df_filtered['Confidence'] = [res['score'] for res in results]
                
                st.dataframe(df_filtered, use_container_width=True)

                # Grafikon
                fig = px.bar(df_filtered['Sentiment'].value_counts(), title="Rezultati analize")
                st.plotly_chart(fig, use_container_width=True)

                # WORD CLOUD - Uvoz samo tukaj!
                st.subheader("☁️ Word Cloud")
                from wordcloud import WordCloud
                import matplotlib.pyplot as plt
                
                text = " ".join(review for review in df_filtered['Komentar'])
                wc = WordCloud(background_color="white").generate(text)
                fig_wc, ax = plt.subplots()
                ax.imshow(wc)
                ax.axis("off")
                st.pyplot(fig_wc)
    else:
        st.warning("Ni podatkov za ta mesec.")
